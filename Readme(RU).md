...
- [🍑 Какие преимущества использования RXCM ?](#какие-приемущества-использования-RXCM-?)
...
- [🔧 Что менять в конфигурационных файлах после изменения содержимого кэша ?](#что-менять-в-конфигурационных-файлах-после-изменения-содержимого-кэша-?)
...
- [💼 Снипеты и дополнительные возможности](#снипеты-и-дополнительные-возможности)
  - [Регулируемый тип интернет соединения](#регулируемый-тип-интернет-соединения)
...
**RXCM** (RXCacheManager) - это класс, который при открытии приложения получает от сервера информацию о состоянии облачного хранилища. Затем **RXCM** сравнивает версию кэша на устройстве и версию хранилища в облаке. После чего совершает обновления. 
...
## Какие преимущества использования RXCM ?
...
Это файл который описывает текущие состояние кэша. 
...
| `minimalCacheVersion` | минимальная версия кэша на устройстве для которой примнимы обновления от новых версий которые храняться в облаке.храниться |
...
| Ключ                                | Объяснение                                                                                                                                                            |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `urlAtListOfURLsToUpdatesByVersion` | ссылка на json который содержит ссылки на модели описывающие изменения по версиям.                                                                                    |
...
Словарь вложенный в массив `changes` имеет симметричный объект - **RXCMOneChange : NSObject**
...
О том как сконфигурировать словарь так чтобы алгоритм RCXM понял что например мы хотим удалить/добавить файл (или совершить другие действия) будет описано позже.
...
Алгоритм RXCM проанализирует состояние переменных объекта и поймет какого рода изменения должно быть выполнено.
...
| Переименование | Переименовывает файл или папку                                                                                                                                                                                                                                                                                                                                                                            |
...
Если вы хотите удалить (файл или папку), то в поле `oldPath` укажите текущие расположение элемента, а поле `newPath` оставьте пустым. Таким образом алгоритм RXCM поймет что вы хотите удалить элемент.
...
Например такой директорией может быть корневая папка вашего репозитория как это сделал я. Пример: https://github.com/m1a7/DemoCacheStorage

#### Шаг 2

Наполните пользовательским контентом папку **RXCacheContent**. И создайте архив **RXCacheContent.zip** который будет располагаться в той же директории.
...
## Что менять в конфигурационных файлах после изменения содержимого кэша ?
...
Перетащите папку вместе с вложенным фрамеворком в структуру проекта.
...
Для этой цели мы рекомендуем выбрать класс **AppDelegate**, потому что объект кэш менеджера должен существовать на протяжении всей жизни приложения + рекомендуется вызывать некоторые методы управления менеджером (например приостановка обновления или возобновления) в стандартных методах **AppDelegate** когда приложение закрывается и открывается.
...
**RXRemoteNetworkCacheManagement** - при выборе этого протокола вы сами должны скачивать все файлы, и предоставлять фраемворку уже готовые файлы. Такой способ взаимодействия может быть полезным если вы уже имеете сложный собственно написанный сетевой слой, и к например во время любых запросов к вашему серверу пользователь должен заполнять графическую капчу.

Также определитесь в какой папке на устройстве вы будите хранить пользовательский кэш.     
Для это цели Apple рекомендует использовать папку `Library/Caches`.
...
| Метод                                                   | Объяснение                                                                                                                                                                                                                                          |
| ------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| - (RXCM_State) startSuspendResumeUpdateProcess;         | Данный метод является аналогом кнопки play на плеере. Когда вы его вызываете, он будет сменять состояние кэша в зависимости от текущего состояния.<br/>Например если процесс обновления сейчас приостановлен, он будет запущен заново. И на оборот. |
...
### 1. Регулируемый тип интернет соединения
...
Если процесс обновления кэш был завершен с ошибкой, то вы можете получить подробную информацию в проперти `errorOccuredAtExecutionUpdate`.
...
**Инициализация прототипа cacheStatus.json и запись в память устройства.**
